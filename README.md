...
R: Para testar foi necessário rodar o comando `dotnet test --filter "Questao02Tests" --logger "console;verbosity=detailed"`

- Tempo médio de inserção em posição aleatória: 16,8 ms
Speedup: A inserção no final é 3,05x mais rápida que inserção em posição aleatória

   Quando inserimos em posição aleatória, o array precisa deslocar todos os elementos após aquela posição para abrir espaço. Quanto mais elementos já existem na lista, mais deslocamentos são necessários, causando degradação progressiva de performance.
